...
public:
	...
	TEST_METHOD(test_lexical_analyzer_digit_hex) {
		program_ptr p = make_shared<program>("main.cpp", "0x7FFFFFFF");
		Assert::AreEqual(1, (int)lexical_analyzer(p).analyze().size());
	}

	TEST_METHOD(test_lexical_analyzer_digit_double) {
		program_ptr p = make_shared<program>("main.cpp", "2.5");
		Assert::AreEqual(1, (int)lexical_analyzer(p).analyze().size());
	}

	TEST_METHOD(test_lexical_analyzer_digit_oct) {
		program_ptr p = make_shared<program>("main.cpp", "0800l");
		Assert::AreEqual(1, (int)lexical_analyzer(p).analyze().size());
	}

	TEST_METHOD(test_lexical_analyzer_digit_scientific) {
		program_ptr p = make_shared<program>("main.cpp", "1e9");
		Assert::AreEqual(1, (int)lexical_analyzer(p).analyze().size());
		p = make_shared<program>("main.cpp", "1.0e+9");
		Assert::AreEqual(1, (int)lexical_analyzer(p).analyze().size());
		p = make_shared<program>("main.cpp", "9.0e-9");
		Assert::AreEqual(1, (int)lexical_analyzer(p).analyze().size());
	}
	...
	TEST_METHOD(test_lexical_analyzer_string)
	{
		Assert::ExpectException<compilation_error>([this]() {
			program_ptr p = make_shared<program>("main.cpp", "#include <iostream>\n"
				"#include <utility>\n"
				"#include <cstdio>\n"
				"using namespace std;\n"
				"// this is a line comment\n"
